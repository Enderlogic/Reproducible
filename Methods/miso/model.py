...
class Miso(nn.Module):
    def __init__(self, features, ind_views='all', combs='all', sparse=False, neighbors=None, device='cpu'):
        ...
        if not self.sparse:
            ...
            self.adj = [torch.sparse.FloatTensor(indices[i], values[i], shape[i]).to(self.device) for i in
                        range(len(adj))]
    ...
    def train(self):
        self.mlps = [MLP(input_shape=self.pcs[i].shape[1], output_shape=32).to(self.device) for i in
                     range(len(self.pcs))]
